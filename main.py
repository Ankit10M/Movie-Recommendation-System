...
if not TMDB_API_KEY:
    raise RuntimeError('API key missing put it in .env')
...
INDICES_MATRIX_PATH=os.path.join(BASE_DIR, 'indices.pkl')
...
class SearchBundleResponse(BaseModel):
    ...
    tfidf_recommnedations:List[TFIDFRecItem]
...
async def tmdb_get(path: str, params: Dict[str, Any])-> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f'TMDB request error: {type(e).__name__} | {repr(e)}',
        )
...
def load_pickles():
    ...
    # Load indices
    with open(INDICES_MATRIX_PATH, "rb") as f:
